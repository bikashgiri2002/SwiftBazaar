...
function Cart() {
  ...
  useEffect(() => {
    ...
    const fetchCartItems = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/users/cart/items", {
      ...
      {cart && cart.products.length > 0 ? (
        <div className="bg-white p-6 rounded-lg shadow-lg">
          {cart.products.map((item, index) => (
            <div key={item.product?._id || `cart-item-${index}`} className="flex items-center justify-between p-4 border-b">
              <div>
                <h3 className="text-lg font-semibold text-gray-800">{item.product?.name || "Unknown Product"}</h3>
                <p className="text-gray-600">Price: <span className="text-gray-900 font-medium">${item.product?.price || "N/A"}</span></p>
          ...
          <div className="mt-6 flex justify-between items-center text-xl font-bold text-gray-800">
            <span>Total:</span>
            <span>${cart.products.reduce((total, item) => total + (item.product?.price || 0) * item.quantity, 0)}</span>
          </div>
